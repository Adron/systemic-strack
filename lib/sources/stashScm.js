...
// TODO: Refactor 200 status code segment.
// TODO: Refactor parsing.
function getRepositoriesProjectHeaders (error, response, body) {
  if (!error && response.statusCode == 200) {
...
// TODO: Refactor 200 status code segment.
// TODO: Refactor parsing.
function getReposWithoutProjectHeaders (error, response, body) {
  if (!error && response.statusCode == 200) {
    var repositories = JSON.parse(body).values;
...
function getRepositories (error, response, body) {
  if (!error && response.statusCode == 200) {
    var projectRepos = helpers.getProjectRepoUrls(JSON.parse(body).values);

    for (var i = 0; i < projectRepos.length; i++) {
      options.url = projectRepos[i];
      request(options, getReposWithoutProjectHeaders);
    }
  }
}
...
function getProjectsRespositories (error, response, body) {
  if (!error && response.statusCode == 200) {
    ...
    for (var i = 0; i < projectRepos.length; i++) {
      options.url = projectRepos[i];
      request(options, getRepositoriesProjectHeaders);
    }
  }
}
...
// TODO: Refactor 200 status code segment.
// TODO: Refactor parsing.
  function getReposList (error, response, body) {
    if (!error && response.statusCode == 200) {
      var projectRepos = helpers.getProjectRepoUrls(JSON.parse(body).values);
